...
#login {
 ...
 function actualizar_intercambio(actual, nuevo, cedulaingreso) 
        {	
		
		if (actual==""){ alert ("La casilla de cedula de la izquierda debe ser diligenciada")
					document.getElementById('actual').focus();
					return;
					}
		if (nuevo==""){ alert ("La casilla de cedula de la derecha debe ser diligenciada")
                ...
                $.ajax({
                data: parametros,
                url: 'http://190.144.42.83:9090/plantillas/suministros/actualiza_intercambio.php',
                type: 'post',
                    beforeSend: function () 
                    {
    ...
    <td>
        <div class="formulario" id="login" style="margin-top: 40px"> 
          <table width="90%" border="1" align="center">
            <tr>
              <th scope="col">Intercambiar elementos entre dos  empleados</th>
            </tr>
          </table>
          <p><br>
          </p>
          <p>
            ...
            <input name="resp_actual" type="text" id="resp_actual" size="15" onChange="llenarnuevo(this.value);" />                    
          ...
          <p align="center" style="color:#039"><span style="text-align: left; font-size: 16px; color: #00C; font-family: Tahoma, Geneva, sans-serif;"><b>
                <input name="nombre_nuevo" type="text" id="nombre_nuevo" size="60" />
          </b><br />
            <br />
            </span>
          
          <p align="center"><input name="generar" type="button" id="generar" onclick="actualizar_intercambio($('#resp_actual').val(),$('#resp_nuevo').val(), '<?PHP echo $cedulaingreso;?>'); return false;" value="ACTUALIZAR" class="botones"/></p>
