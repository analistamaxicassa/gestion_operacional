...
if($empresa=="TUC")
{
	foreach($archivo as $linea_num =>$linea)
	{
			  ...
			  $codigo_numeracion=trim($datos[15]);
			  $transmitido=trim($datos[16]);
			  $concepto=trim($datos[17]);
			  $base_prorrateo=trim($datos[18]);
			  $id_moneda=trim($datos[19]);
			  $tasa=trim($datos[20]);
			  $codigo=trim($datos[21]);
			  $conceptocontable=trim($datos[22]);
			  $idimportacion=trim($datos[23]);
			  $facturaantes=trim($datos[24]);
			  $observaciones=trim($datos[25]);
		  
			  echo $insertar="INSERT INTO gastos1_2000 (numero,
					   mes,
					   anulado,
					   fecha,
					   nit,
...
{   
    foreach($archivo as $linea_num =>$linea)
    {
     ...
     $naturaleza=trim($datos[6]);
     $nit=trim($datos[7]);
     $conceptocontable=trim($datos[8]);
     $valor_moneda=trim($datos[9]);
     $codarancel=trim($datos[10]);
     $numeropedido=trim($datos[11]);
     $noprorratear=trim($datos[12]);
     $base=trim($datos[13]);
     $numerodocumento=trim($datos[14]);
     $concepto=trim($datos[15]);
     $subcentrocosto=trim($datos[16]);
     
     echo $insertar="INSERT INTO gastos2_2000(id,
                numero,
                anulado,
                cuentacontable,
                descripcion,
...
foreach($archivo as $linea_num =>$linea)
    {
     ...
     $numeropedido=trim($datos[11]);
     $base=trim($datos[12]);
     $numerodocumento=trim($datos[13]);
     $noprorratear=trim($datos[14]);
     $concepto=trim($datos[15]);
     $subcentrocosto=trim($datos[16]);
