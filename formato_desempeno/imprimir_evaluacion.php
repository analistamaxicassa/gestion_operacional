...
$link=Conectarse();
 ...
 $sql1="select MAX(fecha) fecha FROM `form_desem_detallado` where cedula = '$cedulae' ";
...
`concepto_gh`,`concepto_auditoria`,`Contrataxempresa`,`condiciones`,`capacitacion` FROM `form_desempeno` WHERE `ced_evaluado` = '$cedulae' and `fecha_evaluacion` = '$rs_qry1->fecha'";
...
<form method="post">
  <div id="validador">
    ...
    <table width="97%" border="1" align="center" style="font-size:12px">
      <tr>
                    <td width="236"  class="encabezados">ASPECTOS </td>
...
$sql="SELECT id, aspecto, definicion FROM `form_def_aspectos` order by id";
...
//Cuenta la cantidad de operaciones por aspecto y genera el porcentaje para cada operacion
	$sql6="SELECT COUNT(`aspecto`) NUMASPECTO FROM `form_desem_detallado` WHERE `aspecto` = '$Qaspecto' and cedula = '$cedulae' and fecha = '$ultimafecha' ";
...
// genera el peso dependiendo del aspecto
	  ...
	  $sql7="SELECT valor FROM `form_def_aspectos` where id = '$Qaspecto'";
...
// realiza formula para calcular la calificacion por aspecto
	 ...
	 $sql8="SELECT  sum(valoracion * $porcentaje)* $pesoxaspecto valoraspecto  FROM `form_desem_detallado`  WHERE cedula ='$cedulae' and fecha = $ultimafecha and aspecto = '$Qaspecto'";
		 ...
		 $valoraspecto = $rs8->valoraspecto;
			...
			?>
                  ...
                  <tr>
                    ...
                    <td width="92" class="encabezados">Valoracion</td>
                    <td width="291" class="encabezados">Aspectos a resaltar </td>
                    <td width="367" class="encabezados">Aspectos a mejorar </td>
                  </tr>

             <?php


			$sqla="SELECT  operacion, valoracion, resaltarop, mejorarop FROM `form_desem_detallado` edll inner join ed_def_aspectos ed on edll.aspecto = ed.id WHERE cedula = '$cedulae' and fecha = '$rs_qry1->fecha' and edll.aspecto ='$rs_qry->id'";
