...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'INFORME DE ASPECTOS A MEJORAR'
        },
        subtitle: {
            text: '<?php
        ...
        xAxis: {
            categories: [
<?php
$sql=mysql_query("SELECT `bloque4`, count(bloque4) qbloque4 FROM `entrevista_retiro` WHERE mes $tipo and anio = $anio and `bloque4` <> '' and empresa = '$empresa' group by bloque4 order by count(bloque4) desc");
while($res=mysql_fetch_array($sql)){			
?>
			
			['<?php echo utf8_encode($res['bloque4']) ?>'],
