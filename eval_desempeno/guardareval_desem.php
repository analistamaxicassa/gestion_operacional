  ...
  <script>
	 ...
	 function paraimpimir(cedulae)
        {	//alert(cedulae);
                ...
                $.ajax({
                data: parametros,
                url: 'http://190.144.42.83:9090/plantillas/eval_desempeno/imprimir_evaluacion.php',
                type: 'post',
                   beforeSend: function () 
                    {
...
// $sql2="SELECT count(id) total FROM `ed_aspectos` WHERE cargo = '$codcargo' "; 
 $sql2="select count(id) total FROM `ed_aspectos` WHERE rol in ('$rol','999') and cargo in ('$codcargo','','999')";
...
// muestra los id de las operaciones dependiendo del cargo y rol definido 
	...
	$sql2a="select id FROM `ed_aspectos` WHERE rol in ('6','999') and cargo in ('101','','999')";
...
$sql5="INSERT INTO `personal`.`eval_desempeno` (`id`, `ced_evaluado`, `ced_evaluador`, `fecha_evaluacion`, `periodo`, `empresa`, `cc`,  `cargo`, `mejoras`, `fseguimiento`, `obs_evaluador`,  `concepto_gh`, `concepto_auditoria`, `Contrataxempresa`, `condiciones`, `capacitacion`) VALUES (NULL, '$cedulae', '$cedulaent', '$hoy', '$periodo', '$sociedad', '$nombrecc', '$cargo',  '$mejoras', '$fseguimiento', '$obs_evaluado', '$concepto_gh', '$concepto_auditoria', '$contrata_empresa', '$condiciones', '$capacitacion');"; 
