...
$encabezado='<center><span style="font-size:24px;; font-weight:bold"><br><br><br><br><br><br>CERTIFICADO DE RETENCION EN LA FUENTE<br><br><br></span></center>';
...
switch($certificado)
{
	case 'P1':
		...
		if (empty($fretiro)) {
			 ...
			 echo $encabezado;
			 echo '<p style="text-align: justify">Que el(a) señor(a)  <b>'.$row['NOMBRE'].' </b> identificado(a) con cedula de ciudadanía No. <b>'. number_format($row['EMP_CODIGO'],0,",",".").'</b>, laboró para la empresa con contrato a   <b>'.$row['TCO_NOMBRE'].'</b> desde el día <b>'.fechaletra($fingreso).' </b> hasta el
...
if ($stmt->execute()) {
  ...
  if (empty($divisor)) {
		 ...
		 echo $encabezado;
		 echo '<p style="text-align: justify">Que el(a) señor(a) <b>'.$nombre.'</b> identificado(a) con cedula de 
...
if ($stmt->execute()) {
    while ($row = $stmt->fetch()) {
	 ...
	 echo '<p align="left">'.$ciudad.', '.fechaletra($hoy).'<br />
	  ...
	  <p align="left">
			  NOMBRE EMPLEADO:<b>         '.$row['NOMBRE'].'</b><br />
...
if ($stmt->execute()) {
	...
	if (empty($fretiro)) {
	 ...
	 echo '<p align="left">'.$ciudad.','.fechaletra($hoy).'<br />
      ...
      <p style="text-align: justify">La  presente con el fin de autorizar el retiro de las cesantías definitivas consignadas en ese fondo del (la) señor (a) <b>'.$row['NOMBRE'].'</b> identificado(a) con cédula de ciudadanía número  <b>'.number_format($row['EMP_CODIGO'],0,",",".").'</b> quien laboró hasta el día
